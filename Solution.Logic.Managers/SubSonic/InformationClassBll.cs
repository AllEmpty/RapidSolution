...
namespace Solution.Logic.Managers {
	...
	public partial class InformationClassBll : LogicBase {
			...
			try
            {
                ...
                switch (conditionColName)
                {
					...
					case "IsSys" :
                        ...
                        break;
					case "ClassImg" :
						model = list.SingleOrDefault(x => x.ClassImg == (string)value);
                        expression = x => x.ClassImg == (string)value;
                        break;
					case "IsShow" :
        ...
        public void SetModelForCache(DataAccess.Model.InformationClass model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.InformationClass>();
		    }
            //添加记录
            list.Add(model);
        }
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<InformationClass>();
			}
        }
		...
		/// <summary>
        /// 删除InformationClass表记录——如果使用了缓存，删除成功后会清空本表的所有缓存记录，然后重新加载进缓存
        /// </summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        ...
        {
            //设置Sql语句
            var sql = string.Format("delete from {0}", InformationClassTable.TableName);
            ...
            {
                //添加用户操作记录
                UseLogBll.GetInstence().Save(page, "{0}删除了InformationClass表所有记录！");
            }
        }
		#endregion
	    ...
	    public void UpdateValue(Page page, int id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, InformationClassTable.Id, Comparison.Equals, id));
            };
		...
		#region 删除ClassImg字段存储的对应图片
		/// <summary>删除ClassImg字段存储的对应图片</summary>
		/// <param name="page">当前页面指针</param>
		/// <param name="pkValue">主键Id</param>
        /// <param name="isCache">是否同步更新缓存</param>
		public void DelClassImg(Page page, int pkValue, bool isCache = true) 
		{
			try {
				string img = "";
				...
				//设置条件
				var wheres = new List<ConditionHelper.SqlqueryCondition>();
				wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, InformationClassTable.Id, Comparison.Equals, pkValue));

				//获取图片地址
				var select = new SelectHelper();
				img = select.GetColumnsValue<InformationClass>(InformationClassTable.ClassImg, wheres) + "";
				...
				//设置更新值
				var setValue = new Dictionary<string, object>();
				setValue[InformationClassTable.ClassImg] = "";
				//更新
                ...
                if (isCache && CommonBll.IsUseCache())
                {
					...
					{
						//给获取的实体赋值
						SetModelValue(model, InformationClassTable.ClassImg, "");
						//更新缓存中的实体
						SetModelForCache(model);
					}
                }
		...
		public int GetSortMax() 
		{
			//查询
			var select = new SelectHelper();
		    return ConvertHelper.Cint0(select.GetMax<InformationClass>(InformationClassTable.Sort));
		}
		#endregion
		...
		public int GetSortMax(int parentId) 
		{
			//定义查询条件
			var wheres = new List<ConditionHelper.SqlqueryCondition>();
			wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, InformationClassTable.ParentId, Comparison.Equals, parentId));
			//查询
			var select = new SelectHelper();
		    return ConvertHelper.Cint0(select.GetMax<InformationClass>(InformationClassTable.Sort, wheres));
		}
		#endregion

                ...
                foreach (string key in hashed.Keys)
                {
                    ...
                    if (col != null)
                    {
                        if (!col.AutoIncrement && !col.IsReadOnly && !(col.DefaultSetting != null && hashed[key] == null))
                            //对添加修改操作增加判断，如果对某一字段没有赋值，则不对其进行更新操作，使用数据库中默认值，另外字段为日期类型时，如果日期为默认最小值，也使用数据库中默认值
                            if (hashed[key] == null || (col.IsDateTime && hashed[key].Equals(new DateTime(0001, 1, 1))))
                                continue;
                            else

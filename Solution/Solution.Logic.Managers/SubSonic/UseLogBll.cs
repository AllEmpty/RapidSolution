...
namespace Solution.Logic.Managers {
	...
	public partial class UseLogBll : LogicBase {
        ...
        public void SetModelForCache(DataAccess.Model.UseLog model)
        {
		    ...
		    if (list == null)
		    {
                list = new List<DataAccess.Model.UseLog>();
		    }
            //添加记录
            list.Add(model);
        }
        ...
        public int GetRecordCount()
        {
			...
			else
			{
				...
				return select.GetRecordCount<UseLog>();
			}
        }
		...
		/// <summary>
        /// 删除UseLog表记录——如果使用了缓存，删除成功后会清空本表的所有缓存记录，然后重新加载进缓存
        /// </summary>
        /// <param name="page">当前页面指针</param>
        /// <param name="expression">条件语句</param>
		/// <param name="isAddUseLog">是否添加用户操作日志</param>
        ...
        {
            //设置Sql语句
            var sql = string.Format("delete from {0}", UseLogTable.TableName);
            ...
            {
                //添加用户操作记录
                UseLogBll.GetInstence().Save(page, "{0}删除了UseLog表所有记录！");
            }
        }
		#endregion
	    ...
	    public void UpdateValue(Page page, int id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
            ...
            if (id > 0)
            {
                ...
                wheres.Add(new ConditionHelper.SqlqueryCondition(ConstraintType.And, UseLogTable.Id, Comparison.Equals, id));
            };
	    ...
	    public void UpdateValue_For_MenuInfo_Id(Page page, int MenuInfo_Id, Dictionary<string, object> dic, string content = "", bool isCache = true, bool isAddUseLog = true)
        {
            ...
            var wheres = new List<ConditionHelper.SqlqueryCondition>
            {
                new ConditionHelper.SqlqueryCondition(ConstraintType.And, UseLogTable.MenuInfo_Id, Comparison.Equals, MenuInfo_Id)
            };
